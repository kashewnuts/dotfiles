...
if type "peco" > /dev/null 2>&1; then
  ...
  case "$OSTYPE" in
    ...
    linux*) function peco-history() {
      local l=$(HISTTIMEFORMAT= history |  sort -k 2 -k 1nr | uniq -f 1 | sort -nr | sed -e 's/^\s*[0-9]*\s*//' | perl -ne 'print unless $seen{$_}++' | peco --query "$READLINE_LINE" | head -n 1)
      READLINE_LINE="$l"
      READLINE_POINT=${#l}
    }
  esac
